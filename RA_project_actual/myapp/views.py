...
# The functions formula3, formula4, and the block of code to calculate water_content, pore_radius, and derivation should remain unchanged.

def input_values(request):
    if request.method == 'POST':
        tetta_s = []
        # Get the number of elements in the array from the user
        num_elements = int(request.POST.get('Quantity of elements', 0))
    ...
    # Now tetta_s contains the cleaned float values

    # Convert the list tetta_s back to a space-separated string
        cleaned_values_string = ' '.join(map(str, tetta_s))

# Update the values variable
        values = cleaned_values_string

        if len(tetta_s) != num_elements:
            error_message = "Number of entered values does not match the specified array size."
            return render(request, 'myapp/input_form.html', {'error_message': error_message})


        # Loop to get user input and store it in the array
        # for i in range(num_elements):
        #     element = float(request.POST.get('Enter element {i+1}:',0))
        #     tetta_s.append(element)
...
# Update the values variable
        ...
        if len(soil_suction) != num_elements:
            error_message = "Number of entered values does not match the specified array size."
            return render(request, 'myapp/input_form.html', {'error_message': error_message})
        ...
        # for i in range(num_elements):
        #     element = float(request.POST.get('Enter element {i+1}:',0))
        #     soil_suction.append(element)
        # # soil_suction = [0.01, 0.0979, 0.1967, 0.4, 0.7, 0.8, 1, 3, 5, 6, 7.5, 10, 20, 40, 59.9964, 80, 100, 200, 300,
        # #                 400, 500, 600, 750, 1000, 1150, 1500, 3000, 5000, 10000, 50000, 100000, 500000, ]
        water_content = []
...
# ]
        ...
        for value in range(num_elements):
            water_content.append(formula3(soil_suction[value], tetta_s[value], a, n, m, soil_suction_r))
        ...
        plt.xscale('log')
        plt.savefig('myapp/static/soil_water_characteristics_curve.png')  # Save the graph as a file
        plt.close()
        ...
        plt.savefig('myapp/static/pore_size_distribution.png')  # Save the graph as a file
        plt.close()
        ...
        plt.xlabel('Soil suction, \u03C8 (kPa)')
        plt.ylabel('Water content, w')
        plt.title("Optimal Soil-water characteristics curve")
        plt.plot(soil_suction, tetta_s, 'x', markersize=10, color="blue")
        plt.xscale('log')
        plt.savefig('myapp/static/optimal_soil_water_characteristics_curve.png')  # Save the graph as a file
        plt.close()
        ...
        plt.xlabel('Pore Radius, r (mm)')
        plt.ylabel('Derivation')
        plt.title("Optimal Pore-size distribution")
        plt.xscale('log')
        plt.xlim(max(pore_radius), min(pore_radius))
        plt.savefig('myapp/static/optimal_pore_size_distribution.png')  # Save the graph as a file
        plt.close()


        print(a,n,m,soil_suction_r)
        context = {
            'quantity':num_elements,
            'tetta_s':values,
            ...
            'optimal_n':optimal_n,
            'optimal_m':optimal_m,
            'optimal_soil_suction_r':optimal_soil_suction_r,
